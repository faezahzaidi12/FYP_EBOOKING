...
(function () {
  ...
  function buildSystemPrompt() {
    ...
    ctx += "2. If start_time is BEFORE 08:00:00 or AFTER 17:00:00, reply directly in text telling user facilities are closed. This rule only applies to action SEARCH_ROOM.\n";
    ctx += "3. If user mentions a SPECIFIC time/hour they want to check (e.g. 'available at 2pm', 'kosong pukul 9'), use action \"SEARCH_ROOM\" with target_room (or null if general) and start_time set to that hour.\n";
    ctx += "4. If user asks GENERALLY whether a specific room has ANY booking/tempahan today, without mentioning a specific hour (e.g. 'DFK 1 ada tempahan tak', 'sekarang ada tempahan ke bilik ni', 'bilik ni free tak hari ni'), use action \"CHECK_ROOM_TODAY\" instead. This checks the WHOLE DAY, not just the current hour.\n";
    ctx += "5. CHECK_ROOM_TODAY REQUIRES a target_room to be specified by the user - if no room is mentioned, ask the user which room they mean instead of outputting JSON.\n";
    ctx += 'Output format for time-specific check: {"action": "SEARCH_ROOM", "booking_date": "YYYY-MM-DD", "start_time": "HH:MM:SS", "target_room": "room name or null"}\n';
    ctx += 'Output format for whole-day check: {"action": "CHECK_ROOM_TODAY", "booking_date": "YYYY-MM-DD", "target_room": "room name"}\n\n';
  ...
  async function handleAIActions(actionObj) {
    ...
    const { action, booking_date, start_time, target_room } = actionObj;

    // ============================================
    // 🆕 ACTION BARU: CHECK_ROOM_TODAY
    // Check SEMUA tempahan untuk satu bilik pada satu tarikh,
    // tanpa kira jam - untuk jawab soalan umum "ada tempahan tak"
    // ============================================
    if (action === 'CHECK_ROOM_TODAY') {
      if (!target_room || target_room === 'null' || target_room.trim() === '') {
        return "🤔 Bilik mana yang awak nak check? Contoh: 'DFK 1 ada tempahan tak hari ni?'";
      }
      ...
      if (error) {
        return "⚠️ Ralat semasa check database: " + error.message;
      }

      const roomBookings = (bookings || []).filter(b => {
        if (b.status && b.status.toLowerCase() === 'cancelled') return false;
        return normalizeRoomName(b.room_name || b.room || b.facility_name || '') === normTarget;
      });

      if (roomBookings.length === 0) {
        return `🟢 <strong>${target_room.toUpperCase()} TIADA sebarang tempahan</strong> pada ${booking_date}.<br>` +
               `Bilik ni kosong sepanjang hari (dalam waktu operasi 8AM-5PM).<br><br>` +
               `📌 <em>Boleh tempah di: <strong>HOME → BOOKING SYSTEM</strong></em>`;
      }

      const slots = roomBookings
        .sort((a, b) => (a.start_time || '').localeCompare(b.start_time || ''))
        .map(b => `${b.start_time || '-'} - ${b.end_time || '-'} (${(b.status || 'pending').toUpperCase()})`);

      return `📋 <strong>${target_room.toUpperCase()} ADA ${roomBookings.length} tempahan</strong> pada ${booking_date}:<br>` +
             `• ${slots.join('<br>• ')}<br><br>` +
             `💡 Slot masa lain di luar waktu-waktu di atas masih available untuk ditempah.`;
    }

    // ============================================
    // ACTION SEDIA ADA: SEARCH_ROOM
    // Check ketersediaan pada satu jam spesifik
    // ============================================
      ...
      try {
        ...
        if (jsonMatch) {
          try {
            ...
            if (actionData.action === 'SEARCH_ROOM' || actionData.action === 'CHECK_ROOM_TODAY') {
